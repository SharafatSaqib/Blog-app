...
export const createPost = async (title, description, content) => {
  ...
  export const fetchPosts = async (page = 1, userId, limit = 5) => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/posts?page=${pageNumber}&userId=${userId}&limit=${pageLimit}`
    );
...
export const fetchAllPosts = async (page = 1, limit = 5) => {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/posts?page=${pageNumber}&limit=${pageLimit}`
  );
